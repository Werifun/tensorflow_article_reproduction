...
class TextRCNN:
    def __init__(self, sequence_length, num_classes, vocab_size, word_embedding_size, context_embedding_size,
        ...
        :param num_classes:  分类的种类
        :param vocab_size: 词表的长度
        :param word_embedding_size: 词向量的长度
        :param context_embedding_size:
        ...
        with tf.device('/cpu:0'), tf.name_scope("embedding"):
            self.W_text = tf.Variable(tf.random_uniform([vocab_size, word_embedding_size], -1.0, 1.0), name="W_text")  # 随机建立词表吗?
            self.embedded_chars = tf.nn.embedding_lookup(self.W_text, self.input_text)  # 转为词向量
        ...
        with tf.name_scope("bi-rnn"):
            ...
            (self.output_fw, self.output_bw), states = tf.nn.bidirectional_dynamic_rnn(cell_fw=fw_cell,
                                                                                       cell_bw=bw_cell,
                                                                                       inputs=self.embedded_chars,  # 词向量输入
                                                                                       sequence_length=text_length,  # 句子的长度
                                                                                       dtype=tf.float32)

        with tf.name_scope("context"):  # 没有理解形状是怎么确定的
        ...
        with tf.name_scope("max-pooling"):  # 怎么就变pool了
